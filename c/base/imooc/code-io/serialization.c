...
int SaveSettings(Settings *settings, char *settings_file) {
  ...
  if (file) {
    ...
    fwrite(&settings->front_size, sizeof(settings->front_size), 1, file);
    fclose(file);
    return OK;
  } else {
...
void LoadSettings(Settings *settings, char *settings_file) {
  ...
  if (file) {
    ...
    fread(&settings->front_size, sizeof(settings->front_size), 1, file);
    fclose(file);
  } else {
...
void LoadSettings2(Settings *settings, char *settings_file) {
  FILE *file = fopen(settings_file, "rb");
  if (file) {
...
int main() {
  Settings settings;
  LoadSettings2(&settings, "data/setting.bin");
  PrintSettings(&settings);
  settings.front_size = 40;
  settings.allow_notification = 30;
  SaveSettings2(&settings, "data/setting.bin");
  return 0;
}
